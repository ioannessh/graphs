...
class SingleSourceBFS:
    ...
    def run_bfs(self, source_vertex):
        schema = StructType([
        ...
        frontier = self.spark.createDataFrame([(int(source_vertex),0,)], ["vertex","dist"]).persist(StorageLevel.MEMORY_ONLY)
        visited = self.spark.createDataFrame([(int(source_vertex),0,None,)], schema=schema).persist(StorageLevel.MEMORY_ONLY)
